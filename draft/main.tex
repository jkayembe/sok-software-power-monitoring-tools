...
\begin{itemize}
    \item   \iness{Iness : Is there a sound reason for the existence of so many of these tools?} \\
            ...
            \jay{Jay : I do agree with the fact that it should be an outcome of the study. To me the classification we create really defines a space of usage scenarios, i.e. needs + constraints. For instance a tool like Kepler might have been designed for data centres operators to monitor energy usage per computing loads and allow energy-aware orchestration of the latter. This translate to a context and its constraints: $(granularity=container/VM, measurment\_target=(CPU, DRAM, storage), measurment\_approach=external\_power\_meters + RAPL, virtualisation\_support, etc...)$. Another use case of a user wanting to assess the energy of a program running on its smartphone would imply another context, other constraints so another point in that space for which there might or not exist a tool}
    \item   \jt{JT : We might want to add a column for use cases of the tools. The value of such a paper might be different for industry people than us academic fellows or even governance people makinig standards etc...}\\
            \iness{Iness : The value of this SoK from these different perspectives could be made more explicit in the introduction to strenghten the scientific contribution}.

    \item \jt{JT : For the moment, it is unclear how the GMT differentiates itself from Kepler. Why coudn't the GMT be used in data centre context? }\\
          \jay{Jay : I think the GMT could be used in a data centre context, provided the DC operator allows access to some metrics (number of CPUs,RAM,... + system-level consumption wether it's real-time or spec-based) as source of truth, and a metric provider (GMT data reporter) is implemented to read and process that data. Such a metric provider exists when running different containers, that uses CPU usage \%age as a proxy for system energy allocation to containers.}
