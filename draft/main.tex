...
\begin{itemize}
    \item \textbf{Measurement target.} Tools may focus on specific hardware components such as the CPU, DRAM, GPU, storage devices, network interfaces and display \blue{(never seen yet for display though. But that might be interesting to shed light on)}, or try to estimate system-level consumption.
...
\begin{itemize}
    ...
    \item \textbf{Maintenance status.} Tools may be more or less well maintained, e.g. reported issues solved etc. \blue{To quantify this we could set a treshold on the date of maintenance}
    
    \item \textbf{Documentation quality.} Clear documentation is important to reduce misuse. \orange{Maybe we drop that one because it's hard to assess, OR we just report existing documentation}
