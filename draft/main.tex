...
\begin{itemize}
    ...
    \item Measurement level \& attribution : What is actually measured. \todo{iness: granularity?}
    \item Methodology \& accuracy : How and how well is it measured. \todo{iness; how will the accuracy be measured?}
    \item Invasiveness \& usability : How is the tool implemented. \todo{iness: I wouldn't use the term usability here}
...
\todo{iness: I am not sure I understand the goal of the itemize list below}
\begin{itemize}
    \item \textbf{Measurement target.} Tools may focus on specific hardware components such as the CPU, DRAM, GPU, storage devices, network interfaces and display \jay{(never seen yet for display though. But that might be interesting to shed light on)}, or try to estimate system-level consumption.
    
    \item \textbf{Metrics.} Most tools report energy consumption in joules or average power in watts. Some might also report network data exchanges while others may even report higher-level metrics such as carbon emissions (e.g., Green Metric Tool (GMT)~\cite{kayembeExploringPrivacySecurity2025}) by combining energy estimates with external carbon-intensity data. These derived metrics introduce additional assumptions beyond energy measurement itself.
...
\begin{itemize}
    ...
    \item \textbf{Accuracy validation.} Some tools may provide accuracy validation e.g., \cite{noureddineMonitoringEnergyHotspots2015}. \jay{(This should maybe be part of "Maturity and ecosystem" dimension or maybe both)}
...
\begin{itemize}
    ...
    \item \textbf{Runtime overhead.} Monitoring introduces computational overhead that may perturb the system under study. Tools could vary in whether this overhead is quantified and how significant it is. \jay{Never saw this yet though}
...
\begin{itemize}
    \item \textbf{Bare-metal support.} Some tools are designed primarily for non-virtualised environments with full hardware access. \jay{Does it make sense to detail targeted architecture here? Or do we create a dedicated feature for that?}
    ...
    \item \textbf{Multi-tenancy awareness.} Whether energy attribution assumes exclusive access to hardware resources or explicitly accounts for concurrent workloads and shared infrastructure. \jay{This one is maybe redundant}
...
\begin{itemize}
    ...
    \item \textbf{Maintenance status.} Tools may be more or less well maintained, e.g. reported issues solved etc. \jay{To quantify this we could set a treshold on the date of maintenance}
    
    \item \textbf{Documentation quality.} Clear documentation is important to reduce misuse. \jay{Maybe we drop that one because it's hard to assess, OR we just report existing documentation}
    
    \item \textbf{Community adoption.} Tools may be primarily used in academic research, industry, or both.
    
    \item \textbf{Ecosystem integration.} Integration with monitoring and visualisation platforms (e.g., Prometheus~\cite{centofantiImpactPowerConsumption2024}, GMT~\cite{hoffmannGreenMetricsTool2025}) facilitates deployment at scale.
\end{itemize}
